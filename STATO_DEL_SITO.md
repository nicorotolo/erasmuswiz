...
due lavori che non si somigliano. **2b-1 chiusa e verificata**:
`riscarica-pdf.mjs` riprende i PDF che la Fase 4a aveva censito senza byte,
rispetta robots.txt e la cortesia della raccolta, mette il testo in cache dentro
la pagina e marca i fallimenti in modo permanente. Le sei prove sono state viste
rosse **due volte**: con le rotture di chi ha scritto il codice, e con altre sei
scelte da chi lo ha riletto. **2b-2 consegnata da Codex**
(`leggi-partner.mjs`, tre prove che impacchettano i sette requisiti, e le due
critiche attraversano davvero `applicaCancelli`): nessuna chiave usata, nessuna
chiamata reale, nessun dato toccato — ⚠︎ **ma la rilettura del diff e del prompt
non è ancora stata fatta**, ed è il primo passo della prossima sessione.

**Tre difetti chiusi, tutti trovati costruendo il caso, nessuno rileggendo il
codice.** (1) La raccolta di controllo è morta con uscita 13: il limitatore
prendeva un posto e non lo restituiva quando l'indirizzo non era un URL
assoluto, e dopo sei perdite si bloccava per sempre, con la rete già ferma.
Corretto; il rilancio ha poi retto **5.058 richieste** dove prima moriva a
1.200. (2) **L'estrattore PDF, dichiarato chiuso la mattina, cancellava i
numeri** — e alcuni li alterava: `Rechbauerstrasse 12` usciva
`Rechbauerstrasse 2`. Dentro un `TJ` i numeri sono spostamenti di crenatura e
vanno buttati, ma una stringa di sole cifre è testo, e le due cose erano
appiattite in una sola. Trovato **guardando il dato**, non il codice: nel testo
estratto si leggeva `Fact Sheet 2026/,`. Misura su quattro PDF veri che chi ha
corretto non aveva mai visto: cifre **509 → 781 (+53%)**. È la classe di errore
che questa architettura non intercetta da sola, perché un dato **alterato passa**
il cancello della citazione: il modello cita ciò che gli mandiamo noi.
(3) L'ordine dei cancelli (E3) resta aperto: è materiale della 2b-3.

**Il campo, misurato stasera.** Raccolta portata a **305 cartelle, 246 partner
raggiunti, 5.555 pagine**: due volte e mezzo la soglia dei 100 che il §6.2
chiede. Riscarico PDF: **870 riscaricati, 417 letti**; dopo di esso i partner
con almeno una pagina leggibile sono **244 su 246**, e i PDF valgono il **15,3%**
del testo disponibile. I 419 falliti sono stati divisi per causa su 40
riscaricati apposta: 32 hanno il testo dentro ma con font a codifica propria,
5 sono scansioni, 3 non hanno flussi leggibili. Il rifiuto è quindi onesto, ma
circa **335 file hanno il testo dentro** e la mappa per decodificarlo la porta
il PDF stesso: candidato per la Fase 5, registrato in spec §2 quinquies E8.
...
congelabile, con le quattro decisioni prese dopo misura sul codice (D‑V3.1
reload + chiave di sessione, mai lo zaino che è per-ateneo · D‑V3.2 budget
misurabile in sessione · D‑V3.3 pin fuori dal Tab dentro l'entrata · D‑V3.4
`wizardMete` è cablato davvero, l'entrata lo chiude). **La correzione che ha
giustificato la sessione**: il piano dava per assodato che il raggruppamento
della mappa fosse il collo di bottiglia — misurato con CPU 4× sul dataset
peggiore fa 12,3 ms, e il caso reale dell'entrata non è 1.595 record ma 424.
Poi `6ed11a5`: costruita da Codex `gpt-5.6-sol` con `/codex-build` in due giri
— entrambi fermati dallo stesso muro strutturale (la suite UI dura 5,7 min, il
limite per comando di Codex è 240 s) — e finita dal revisore. **Cinque difetti
trovati dopo il verde**, tre dei quali invisibili a entrambe le suite: la nav
nascosta per tutta l'entrata; la fase retrocessa a «esplorando»; **entrata e
Mete impilate per chi arriva da un link condiviso** (pagina alta 7521px invece
di 800 — è il percorso d'arrivo per cui esiste V1); il «mai più» che non valeva
per chi abbandona l'esito; e una prova nuova che passava per il motivo
sbagliato. 90/90 unit · **40/40 UI** (+7, ognuna verificata per mutazione) ·
banco prestazionale verde 8 volte di fila.

**(SESSIONE 2026-07-29 BIS — REVIEW V4: DUE REGRESSIONI CHIUSE E CODICE
MORTO RIMOSSO.)** La revisione umana ha trovato due difetti non intercettati
dalle 31 prove UI: il contratto ARIA rinominava l'id del saluto e impediva
«Ciao, Nome»; inoltre il ramo pre-bando aveva precedenza sul profilo mancante
e proponeva subito le mete a un nuovo visitatore. Il saluto ora usa la classe
stabile; la missione conserva la cornice onesta «bando non ancora uscito», ma
propone «Completa il profilo» finché il profilo manca e «Esplora le mete» dopo.

Rimossi soltanto lo stato, il render e i CSS esclusivi della card mappa già
tolta dalla Home. Il motore condiviso della mappa e `.mappa-compatta` restano
per Entrata e Mete. Il contratto sopra `tappaCorrente()` ora dichiara
l'eccezione pre-bando già presente nel codice: è uno **scostamento reale dalla
§V4 congelata**, perché la spec non chiedeva di modificare quella funzione,
ma evita che la chiusura storica del 2026/27 spedisca agli Esiti chi segue il
2027/28.

Nuovo `test/ui/v4-regressioni.spec.cjs`, senza cambiare prove già verdi o
baseline. Entrambe le regole sono state verificate separatamente per
mutazione: selettore saluto riportato all'id → rosso; profilo mancante forzato
a falso → rosso; dopo ciascun ripristino → verde. **Prove finali: 90/90 unit,
33/33 UI.**

**(SESSIONE 2026-07-29 — V4 HOME «ADESSO» + STATO PRE-BANDO COMPLETATA
NEL WORKING TREE. G1 SUPERATO.)** Implementata la specifica congelata §V4
senza delega, senza operazioni Git, senza processi terminati e senza toccare
`statoBando()` o i file `dati-mete-*`. `modoCiclo()` è puro e
`inPreBando()` è l'unico lettore nell'app. I 19 punti di render dichiarano il
ciclo storico con etichetta, passato o disattivazione spiegata; T5 ora contiene
19 asserzioni DOM, non ha `skip` ed è stato provato per mutazione sul punto 9
(cartellino tolto → rosso, ripristino → verde).

La Home segue l'inventario «Adesso»: niente claim, riepilogo-mappa o link
profilo nel progresso; su mobile mostra la sola tappa corrente prima della
mossa principale. A 390×844 badge, tappa, finestra attesa e CTA «Esplora le
mete» stanno nel primo schermo. La chiusura del bando 2026/27 non manda più
un nuovo percorso 2027/28 alla tappa Esito.

Le 18 voci condizionali post-selezione hanno ricevuto solo il campo `tipo`
prescritto: 3 `condizione`, 8 `opzione`, 7 `avvertenza`. Le avvertenze sono
nel riquadro «Da sapere prima» senza checkbox; le opzioni restano spuntabili
ma fuori conteggio; il denominatore comprende solo azioni universali e
condizioni attivate dal profilo. Aggiunte al profilo le due risposte
`extraUE` e `ricercaTesi` a tre valori, conservate da entrambi gli scrittori.
**Prove finali: 90/90 unit, 31/31 UI.**
...
ora comprende uno o due livelli più l'ateneo finale senza registrare schermate
future. `#learning-agreement/sapienza` carica a freddo il solo dataset Sapienza
ma non scrive `erasmuswiz_ateneo`; la rotta non ancora registrata ripiega su
Oggi. Le quattro section hanno un nome accessibile e ricevono il fuoco prima di
un unico scroll, rispettoso di `prefers-reduced-motion`; Indietro/Avanti segue lo
stesso contratto. Ripristinata la scorciatoia di prodotto «Cambia ateneo»:
drawer chiuso → Profilo → fuoco e scroll centrato sulla tendina. Censiti i due
proprietari del fuoco che navigano (drawer e celebrazione): entrambi chiudono
prima della rotta; il dettaglio meta non avvia rotte.

Playwright è ora un gate reale: `playwright.config.cjs`, un solo Chromium e un
server Node statico sulla 8123, avviato e fermato esclusivamente dalla suite.
`npm run test:unit`: **33/33**; `npm run test:ui`: **14/14**. Le UI prove coprono
8 avvii a freddo, dataset Sapienza senza persistenza, 10 navigazioni + 10
Indietro + 10 Avanti, F9 del drawer, `:focus-visible`, reduced motion e le sei
invarianti DOM esistenti sui 4 tab a 390/768/1280. `css/style.css` e i dati non
sono stati toccati. Emersa un'incoerenza nella baseline: il README dichiara
...
separati: `ee2aca0` F3a, `db8d67b` F3b. `index.html` resta intatto.)**

**F3a — la timeline `ol.stazioni`.** Il modello è una **corsia** larga 38px a sinistra
della `<li>`: dentro stanno marker e binario, la card comincia dopo. Così il binario non
passa mai sotto la card e non serve litigare né con lo z-index né con l'`overflow:hidden`
di `.stazione-dettagli`. Eliminati `.stazione + .stazione::before` e
`.stazione-fatta + .stazione::before` (V3): sommandosi al binario nuovo avrebbero
prodotto due tratti sfalsati. Le due guardie obbligatorie sono commentate in loco —
niente `position`/`transform`/`filter`/`will-change` su `.stazione-dettagli` (o il marker
rientra nella card e sparisce), e `box-sizing: border-box` su `.stazione-punto` (o il
bordo da 2px sposta il centro del marker fuori dall'asse del binario).
**Cinque scostamenti dal canvas, tutti misurati.** (1) **Il "binario continuo" del canvas
non è continuo**: con `bottom: calc(-1 * var(--space-4))` il tratto finisce al bordo
superiore della `<li>` successiva, non al suo marker, e lasciava **un buco di 14px
esattamente sopra ogni punto**. La geometria è ora espressa in funzione di `--marker`, e
il blocco desktop cambia **un solo numero**. (2) **`#stazione-mete-ponte` è l'unica `<li>`
senza marker né `<details>`** (index.html:260): il blocco del canvas la ignora e la linea
si spezzava proprio lì → il suo binario parte da `top: 0`, e resta verde se la stazione
precedente è completata. (3) **Bianco su `#10B981` sta a 2,6:1** → si usa `--green`
(#0f7a3a, **5,5:1**), stesso criterio della deroga 4 di F2; idem per il binario (2,4:1
contro 5,1:1). (4) **Tolto l'alone indaco fisso** sotto il marker attivo: con l'anello di
focus installato in F2 sarebbe indistinguibile da un elemento a fuoco. (5) **La tappa
corrente passa da oro a indaco**, con la pillola di stato al seguito — è la grammatica che
F2 ha già dato a `.voce-checklist-v2.attiva`; lasciandone metà, la stazione corrente
sarebbe rimasta mezza oro e mezza indaco.
**Un difetto trovato solo alla misura, non nel diff** (come in F1): con `padding-top: 0`
sul corpo — che è ciò che il canvas prescrive — le stazioni il cui primo figlio è un
banner `display:none` avevano il contenuto vero **appiccicato alla testa, 0px misurati**.
È la trappola di R39: `> :first-child` non sa nulla di `display:none`. Lo stacco ora lo dà
il corpo, 12px in tutte e 5 le stazioni in ogni combinazione di banner acceso/spento.

**F3b — lo stato vuoto ricco.** Helper `creaStatoVuoto(titolo, spiegazione, azione)` +
i due siti in `renderMete()` (ex 1890 e 1894), con i **`return` conservati**: senza,
lista vuota e stato vuoto verrebbero renderizzati insieme. Correzioni al canvas:
**R21** — l'azione "Mostra tutte le mete" cercava `.chip-filtro[data-filtro="tutte"]`,
attributo che non esiste (app.js:1842), quindi il bottone sarebbe nato morto: si agisce
sullo stato, come fanno i chip. Stesso criterio su "Svuota la ricerca", `renderMete()`
diretta invece di un `Event("input")` simulato che passerebbe dal debounce di 150ms.
**R22** — `role="status"` sul nodo, che nasce a runtime dopo un'interazione. **R23** —
nessuna promessa di compatibilità su `.stato-vuoto-v2`: quella classe non ha mai avuto
stile e ora non è più emessa da nessuna parte. In più, il filtro **"lingua" non è una
categoria di compatibilità** e ha una sua variante di copy, o la spiegazione generica
direbbe la cosa sbagliata. `.schedina-invito-vuota` **non** usa l'helper (P1.4, invito di
una riga per scelta deliberata): si allinea solo lo stile, e da riga grigia diventa invito
d'oro — è un suggerimento, non un errore. Niente animazione d'ingresso: la griglia si
ri-renderizza a ogni battuta della ricerca.

**Verifiche.** Marker e binario allineati a **0.00px** a 390 e 1280, **zero buchi** fra i
tratti, marker fermo all'apertura/chiusura (dx 0, dy 0), ultimo tratto spento. Entrambi
gli stati vuoti provati alle tre larghezze: box a tutta griglia (delta 0), mascotte
caricata, bottoni 48px, zero testo tagliato, zero sbordamenti; i due bottoni d'azione
fanno quello che dicono. Contrasti: titolo 16,8:1 · testo 9,1:1 · bottone 8,9:1 ·
schedina vuota 4,5:1. Sonda invarianti F0 verde sui 4 tab a 390/768/1280, touch sotto
44px = le sole 4 eccezioni di `baseline/README.md`. `node --check` OK e **`_smoke.js`
identico al baseline** (392 / 0 / n.d. / 9), zero errori in console. Il detector di
`/impeccable` segnala 13 antipattern, **nessuno introdotto da F3**: sono i 9 `border-left`
colorati di F2 — cioè, indipendentemente, la stessa cosa che le note di F2 avevano già
messo all'ordine del giorno del GATE 2.
⚠︎ **Nota operativa per le prossime prove:** `_smoke.js` richiede **jsdom@22** installato
**fuori dal repo** (da jsdom 28 `ResourceLoader` non è più esportato) e un server con
backlog ampio sulla **8123** — `python -m http.server` ha `request_queue_size = 5` e
rifiuta le connessioni di jsdom, rendendo il baseline instabile (275/392/405 fra run).

**🚦 GATE 2 — validato "per ora" da Nicola (25/07), con revisione completa rimandata.**
Nicola ha dato l'ok a tutto (timeline, stati vuoti, e il passaggio della tappa corrente da
oro a indaco), ma ha esplicitamente rinviato la revisione d'insieme: **«poi rivedremo
tutto insieme alla fine»**. Restano quindi **aperte** le due domande lasciate da F2 —
`--fs-hero` su `.home-hero-claim` (sembra un lapsus del canvas) e se il filetto di stato a
sinistra sia ormai troppo — e non è stato ancora dato un giudizio a video su tipografia,
card, nav, form e modal alle tre larghezze. ⚠︎ **Da fare prima di considerare chiuso il
redesign.**
**Prossimo passo: F4** (checklist §07 in 9 punti, aggiornamento documenti, commit finale)
— ma la revisione d'insieme va fatta prima o insieme.
...
e sparisce dall'URL. Da V1 il parser comprende anche uno/due livelli con ateneo
finale, ma una forma compresa non diventa una rotta finché la sua schermata non
è registrata. Si naviga SOLO con `vaiA(dest, { storia, scroll, forzaFuoco })` —
...
| `test/modo-ciclo.test.cjs` | test unitario | **V4:** tutti i quattro stati × cicli uguali/diversi, più campi mancanti o vuoti |
| `test/inventario-g1.test.cjs` | test unitario + DOM | **V4:** T1–T5; T5 senza skip con 19 asserzioni numerate sui render pre-bando, verificato per mutazione |
| `test/migrazione-zaino.test.cjs` | test unitario | **V2:** migrazione v2→v3 e idempotenza. **V4:** conservazione idempotente di `extraUE`/`ricercaTesi` |
...
| `scripts/leggi-partner.mjs` (attese 5xx) | Fase 7, Passo 1c | Il 5xx ha attesa crescente e contatore proprio, separato dal 429; dopo 8 partner consecutivi falliti la catena si ferma pulita invece di consumare la coda. Senza, un sovraccarico passeggero di Google costa 175 partner in un giro |
...
| `HANDOFF_CHATGPT.md` | trasversale | Briefing autosufficiente da incollare a un secondo modello (ChatGPT) che **non vede il repository**: progetto, catena della mappatura, regole del registro dei giudizi, cancelli, numeri veri, trappole note, cinque difetti aperti, decisioni che restano di Nicola, e cosa non proporre. ⚠️ La sezione dei numeri invecchia: ricontrollarla prima di incollarlo |
...
**Fase 4b, Consegne 2b-1 e 2b-2, la sera del 30/08.** La cache di lettura vale
ora **305 cartelle, 246 partner raggiunti, 5.555 pagine**, e dopo il riscarico
dei PDF **244 partner su 246 hanno almeno una pagina leggibile**: la prova sul
campo del §6.2 chiede 100 partner, e ce ne sono due volte e mezzo tanti. I PDF
valgono il 15,3% del testo disponibile — non un dettaglio, perché è lì che
stanno le factsheet con lingua e scadenze.

**La lezione della sera, che vale oltre questa fase: le prove verdi non vedono
un dato alterato.** L'estrattore PDF era stato dichiarato chiuso la mattina, con
205 prove verdi e tre PDF reali come banco. La sera, guardando il **testo
estratto** invece del codice, si leggeva `Fact Sheet 2026/,` e
`Rechbauerstrasse 2, A- Graz`: cancellava i numeri, e il civico 12 era diventato
2. Un dato così **passa il cancello della citazione**, perché il modello cita
ciò che gli mandiamo noi, e arriverebbe sbagliato allo studente — una scadenza
senza giorno è peggio di una scadenza mancante. Recupero misurato su quattro PDF
veri che chi ha corretto non aveva mai visto: cifre 509 → 781 (+53%). Regola
operativa che si aggiunge alle altre: *dopo ogni estrazione, leggi il testo
estratto con gli occhi, non solo le asserzioni*.

**I falliti si dividono per causa, sempre.** Dei 419 PDF illeggibili, un
campione di 40 riscaricati apposta dice: 32 hanno il testo dentro ma con font a
codifica propria, 5 sono scansioni, 3 non hanno flussi leggibili. Il rifiuto è
onesto — nessun PDF leggibile è stato buttato per un difetto nostro — ma quattro
quinti hanno il testo dentro, e la mappa per decodificarlo la porta il PDF
stesso: è il giacimento più grosso trovato finora, ed è materiale della Fase 5
(spec §2 quinquies E8).
...
**V4 non aggiorna i dati del bando: li rende onesti durante l'attesa.** Tutti i
dati di candidatura restano 2026/27, ma i 19 punti censiti li presentano come
storici quando `cicloPercorso` punta al 2027/28. Le checklist post-selezione
restano 37 voci Ca' Foscari e 31 Sapienza; sono cambiati soltanto il campo
`tipo` sulle 18 voci già condizionali e il modo in cui l'interfaccia le conta.
La review V4 non ha riscritto alcun contenuto validato o dato: ha ripristinato
...
**V1 non modifica contenuti o dati.** Tutti i conteggi della tabella seguente
restano invariati; cambia soltanto il modo in cui un link profondo sceglie il
dataset per quel caricamento.
...
studi. `rootPresunta` prevale sempre e resta fuori dal verde. I vecchi conteggi
del profilo perfetto (940/740) precedono la riduzione 121→71 e non vanno più
citati: si rigenerano con `scripts/conta-anomalie-lingua.mjs` quando servono.
...
76 mete**. Restano **71 `rootPresunta` non revisionate**, debito dichiarato da
...
**⇢ AGGIORNAMENTO 2026-07-29 — questa lista è governata da `PLAN_REDESIGN_V3.md`.**
...
3. ✅ **V4 Home «Adesso» + stato pre-bando COMPLETATA e ONLINE** (`68f571e`,
   ...
   sul lotto notturno.
4. ✅ **V3 Entrata a tutta pagina COMPLETATA e ONLINE** (`6ed11a5`, 29/07),
   con **tre mesi e mezzo di anticipo** sulla scadenza del 15 novembre.
...
5. ✅ **V5 Retention COMPLETATA e RIVISTA** (29/07), spec congelata `cded7ac`.
   ...
   **Chiusi in revisione**: la regressione dello smistamento dell'entrata
   ...
   (36→43 mete).
6. ✅ **V6a — La lista delle mete COMPLETATA e ONLINE** (`3d86900`, 02/08),
   spec congelata `596574c` il 30/07. Lista illimitata e ordinabile, riordino da
...
> 🟡 **LE DUE PROVE DI NICOLA: FATTE A METÀ (aggiornato il 02/08).**
...
0. **F0, F1, F2 e F3 sono FATTE.** Il redesign v2 è implementato per intero.
   ...
   più `e62de58` (esiti di F3 nel piano).
1. ⚠︎ **La revisione d'insieme NON è stata fatta.** Il 25/07 Nicola ha
   validato F3 «per ora» e ha rimandato: *«poi rivedremo tutto insieme alla
   fine»*. Quindi **tipografia, card, nav, form e modal non hanno ancora un
   giudizio a video** alle tre larghezze, benché siano online da F2. Da
   portare a quella revisione anche le **due domande aperte da F2**:
   `--fs-hero` su `.home-hero-claim` (non applicato: sembra un lapsus del
   canvas, il consumatore naturale sarebbe `.home-saluto`) e se il
   **filetto di stato a sinistra** sia ormai troppo — il detector di
   `/impeccable` segnala 9 `border-left` colorati ≥3px, tutti di F2.
   E la novità più visibile di F3: **la tappa corrente da oro a indaco**.
2. **F4** — i 9 punti della checklist §07 con le tre correzioni del piano
   ("griglia nei tre stati" alla luce di R3; "nessun target sotto 44px" col
   criterio **lista vuota**, già soddisfatto da F2; il punto tipografia
   spetta alla revisione d'insieme). Sospetti dichiarati per lo scroll
   orizzontale: `.schedina-nome` senza `min-width:0` e la riga di chip
   filtro. Poi commit finale.
3. ⚠︎ **F3 non è ancora pubblicato**: `ee2aca0`, `db8d67b` e `e62de58` sono
   **solo in locale**. Online c'è ancora la timeline vecchia. Il push va
   deciso da Nicola.
4. **Nota operativa per chiunque rifaccia le prove**: `_smoke.js` richiede
   **jsdom@22** installato **fuori dal repo** (da jsdom 28 `ResourceLoader`
   non è più esportato) e un server con backlog ampio sulla **8123** —
   `python -m http.server` ha `request_queue_size = 5`, jsdom apre ~20
   connessioni insieme e il baseline diventa instabile. Baseline sano:
   **METE 392 · Area 0 · SCADENZE n.d. · CHECKLIST 9 · card 0**, con una
   eccezione **pre-esistente** (`SCADENZE_CAFOSCARI is not defined`, solo
   sotto jsdom: in browser reale la console è pulita).

**Aggiornamento 2026-07-25 (F2 chiusa) — storico:**
